...
contract MISTxStaking is Ownable, IERC721Receiver{
    ...
    MistGovernance private nftToken;
    ...
    constructor(address _mistTokenAddress, address _nftContractAddress, uint _rewardRate, uint _rewardInterval, uint _stakingFeeRate, uint _unstakingFeeRate, uint _cliffTime, uint _minSSGTForGov) {
        nftToken = MistGovernance(_nftContractAddress);
    ...
    function getSSGTGovernanceTokenAddress() public view returns(MistGovernance){
        return nftToken;
    }
    ...
    function mintNFTToken(uint amountToStake) internal {
    ...
    function deposit(uint amountToStake) public {
        ...
        require(IERC20(mistTokenAddress).transferFrom(msg.sender, address(this), amountToStake), "Insufficient Token Allowance");

        // updateAccount(msg.sender);
        
        mintNFTToken(amountToStake);

        uint fee = amountToStake.mul(stakingFeeRate).div(1e4);
        require(IERC20(mistTokenAddress).transfer(owner, fee), "Could not transfer deposit fee.");
    ...
    function withdraw(uint amountToWithdraw) public {
        ...
        require(IERC20(mistTokenAddress).transfer(owner, fee), "Could not transfer withdraw fee.");

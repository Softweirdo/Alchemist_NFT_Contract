...
contract OWLNFT is ERC721Enumerable, Ownable{
    ...
    function addTokenCharacteristicsInBulk(uint[] memory _tokenId, uint[] memory _level, string[] memory _breed, string[] memory _claw, string[] memory _wingspan, string[] memory _sight) public onlyOwner{
        require(_tokenId.length == _claw.length, "parameter missing");
        require(_tokenId.length == _wingspan.length, "parameter missing");
        require(_tokenId.length == _sight.length, "parameter missing");
        require(_tokenId.length == _level.length, "parameter missing");
        require(_tokenId.length == _breed.length, "parameter missing");
        for(uint i = 0; i < _tokenId.length; i++){
            addTokenCharacteristics(_tokenId[i], _level[i], _breed[i], _claw[i], _wingspan[i], _sight[i]);
        }
    }
    
    function addTokenCharacteristics(uint _tokenId, uint _level, string memory _breed, string memory _claw, string memory _wingspan, string memory _sight) public onlyOwner{
        tokenCharacteristics[_tokenId].claw = _claw;
        tokenCharacteristics[_tokenId].wingspan = _wingspan;
        tokenCharacteristics[_tokenId].sight = _sight;
...
contract MISTxStaking is Ownable, IERC721Receiver{

    using SafeMath for uint;


    //@update NFT contract instance
    OWLNFT private nftToken;

    event RewardsTransferred(address holder, uint amount);

    // mist token contract address
    address private  mistTokenAddress;
    ...
    constructor(address _mistTokenAddress, address _nftContractAddress) {
        nftToken = OWLNFT(_nftContractAddress);
        mistTokenAddress = _mistTokenAddress;
    }
     ...
     * @return MISTTokenAddress of erc20 token address which is stake in this contract
     */
    function getMISTTokenAddress() public view returns(address){
        return mistTokenAddress;
    }
    
    function getMISTGovernanceTokenAddress() public view returns(OWLNFT){
        return nftToken;
    }
